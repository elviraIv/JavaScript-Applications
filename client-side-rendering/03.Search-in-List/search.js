...
// template: ul -> highlight elements based on search result
const listTemplate = (towns) => html`
 <ul>
    ${towns.map(t => html`<li class=${t.match ? 'active' : ''}>${t.name}</li>`)}
</ul>`;

// start: load and parse data -> call update -> add event listener to search field
...
// on search: read input value -> compare with town names and modify data -> output result -> call update 
function onSearch(){
